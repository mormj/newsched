...
extern void
apply_passthrough(cuFloatComplex* in, cuFloatComplex* out, int grid_size, int block_size);
 ...
 */
passthrough::passthrough(const size_t batch_size) : gr::sync_block("passthrough"), d_batch_size(batch_size)
...
work_return_code_t passthrough::work(std::vector<block_work_input>& work_input,
                             std::vector<block_work_output>& work_output)
{
    ...
    for (auto s = 0; s < noutput_items; s++) {
        ...
        apply_passthrough(in_data, out_data, d_batch_size / d_block_size, d_block_size);
