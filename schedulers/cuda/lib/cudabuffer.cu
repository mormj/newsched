...
cuda_buffer::sptr cuda_buffer::make(
    size_t num_items,
    size_t item_size,
    buffer_position_t buf_pos = buffer_position_t::NORMAL) // cuda_buffer_type type)
{
    if (buf_pos == buffer_position_t::INGRESS) {
        return cuda_buffer::sptr(
            new cuda_buffer(num_items, item_size, cuda_buffer_type::H2D));
    } else if (buf_pos == buffer_position_t::EGRESS) {
        return cuda_buffer::sptr(
            new cuda_buffer(num_items, item_size, cuda_buffer_type::D2H));
    } else {
        return cuda_buffer::sptr(
            new cuda_buffer(num_items, item_size, cuda_buffer_type::D2D));
    }
}
