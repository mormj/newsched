...
int main(int argc, char* argv[])
{
    ...
    desc.add_options()("help,h", "display help")(
        ...
        "domain_adapt",
        po::value<int>(&domain_adapt)->default_value(0),
        "Domain Adapter Types")("rt_prio", "Enable Real-time priority");
...

    {
        ...
        auto fanout = blocks::fanout::make(sizeof(gr_complex) * veclen, nblocks);
        ...
        if (buffer_type == 0) {
            ...
            for (int i = 0; i < nblocks; i++) {
                if (domain_adapt == 1) {
                    fg->connect(head, 0, fanout, 0);
                    fg->connect(fanout, i, copy_blks[i], 0);
                } else {
            ...
            for (int i = 0; i < nblocks; i++) {
                if (domain_adapt == 1) {
                    fg->connect(head, 0, fanout, 0, VMCIRC_BUFFER_ARGS);
                    fg->connect(fanout, i, copy_blks[i], 0, VMCIRC_BUFFER_ARGS);
                } else {
        ...
        if (domain_adapt == 0) {
            ...
            std::vector<std::shared_ptr<schedulers::scheduler_st>> scheds(nblocks * 2 +
                                                                          3);
            for (int i = 0; i < 3 + 2 * nblocks; i++) {
            ...
            dconf.push_back(domain_conf(scheds[2 * nblocks + 2], { fanout }, da_conf));
